-- SubManager core schema. All currency stored as integer cents.
-- Mirrored by app/lib/db/migrations.ts (inline string for Worker runtime — see
-- engineering convention "no ?raw imports for SQL on the Worker").
...
CREATE INDEX IF NOT EXISTS idx_renewal_reminders_scheduled ON renewal_reminders(scheduled_for) WHERE sent_at IS NULL;
